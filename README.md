...
This project demonstrates a complete workflow for developing, testing, and deploying applications using multiple technologies. It covers:

- **Gitea & MySQL on Kubernetes**:  
  Deploy a Gitea instance and a MySQL database with secure credentials, persistent storage, and an Ingress for external access.

- **Docker Hub Integration in CI/CD**:  
  Authenticate with Docker Hub, build/tag/push Docker images, and update Kubernetes deployments automatically.

- **Flask Database Health Check Service**:  
  A Flask application that checks whether a MySQL database is accessible on port 3306, verifying connectivity, authentication, and service status.

- **Gitea Runner in Kubernetes**:  
  Deploy a dedicated CI/CD runner with Docker socket access to handle Gitea Actions jobs.

- **CI/CD Pipeline for MySQL Alive Container Deployment**:  
  A pipeline that generates a unique tag from the Git commit, builds and pushes a `mysql-alive` image to Docker Hub, and updates the Kubernetes deployment manifest with the new image.

- **Enhancing Gitea Deployment on Kubernetes**:  
  Adding liveness/readiness probes, resource limits, and configuring the metrics-server to ensure that Gitea runs stably under load.

- **Running the CI/CD Pipeline on feat-branch in Gitea**:  
  Configuring the pipeline to trigger on feature branches and addressing connectivity issues using the Node IP.
...
- [Deployment and Configuration Steps](#deployment-and-configuration-steps)
  - [1. Deploying Gitea & MySQL on Kubernetes](#1-deploying-gitea--mysql-on-kubernetes)
  - [2. Enhancing Gitea Deployment on Kubernetes](#2-enhancing-gitea-deployment-on-kubernetes)
  - [3. Configuring Docker Hub Authentication in Gitea CI/CD](#3-configuring-docker-hub-authentication-in-gitea-cicd)
  - [4. Flask Database Health Check Service](#4-flask-database-health-check-service)
  - [5. Setting Up a Gitea Runner in Kubernetes](#5-setting-up-a-gitea-runner-in-kubernetes)
  - [6. CI/CD Pipeline for Database Health Check Service](#6-cicd-pipeline-for-database-health-check-service)
  - [7. CI/CD Pipeline for MySQL Alive Container Deployment](#7-cicd-pipeline-for-mysql-alive-container-deployment)
  - [8. Running the CI/CD Pipeline on feat-branch in Gitea](#8-running-the-cicd-pipeline-on-feat-branch-in-gitea)
